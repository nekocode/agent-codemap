...
sync_versions() {
    ...
    for pkg_dir in "$NPM_DIR"/agent-codemap*/; do
    ...
    local main_pkg="$NPM_DIR/agent-codemap/package.json"
    for platform in darwin-arm64 darwin-x64 linux-x64 win32-x64; do
        if [[ "$(uname)" == "Darwin" ]]; then
            sed -i '' "s/\"@nekocode\/agent-codemap-$platform\": \".*\"/\"@nekocode\/agent-codemap-$platform\": \"$version\"/" "$main_pkg"
        else
            sed -i "s/\"@nekocode\/agent-codemap-$platform\": \".*\"/\"@nekocode\/agent-codemap-$platform\": \"$version\"/" "$main_pkg"
        fi
    done
...
for platform in darwin-arm64 darwin-x64 linux-x64 win32-x64; do
    pkg_dir="$NPM_DIR/agent-codemap-$platform"
    pkg_name="@nekocode/agent-codemap-$platform"
    # Windows uses .exe extension
    if [[ "$platform" == "win32-x64" ]]; then
        binary="$pkg_dir/bin/agent-codemap.exe"
    else
        binary="$pkg_dir/bin/agent-codemap"
    fi
    if [[ -f "$binary" ]]; then
